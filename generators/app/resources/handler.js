...
module.exports = class PromptHandler {
  ...
  secondPrompt(answers) {
    ...
    if (answers.subDirConfim) {
      return this.prompt(secondPrompt).then(res => {
  ...
  thirdPrompt(answers) {
    return this.prompt(installPrompt).then(res => {
  ...
  setPlugins(answers) {
    return this.prompt(pluginsPrompt).then(res => {
  ...
  finishPrompts(answers) {
    ...
    const plugins = answers.plugins,
      ...
      configPath = this.templatePath(
        `${this.language}/src/config/_index.${this.ext}`
      ),
      configNewPath = this.templatePath(
        `${this.language}/src/config/index.${this.ext}`
      ),
    ...
    if (_.keys(answers).length > 0) {
      ...
      configBuffer = configFile.replace(
        hook,
  ...
  removingUnnecessaryFiles() {
    ...
    //app files
    this.fs.delete(
      this.templatePath(`${this.language}/src/app/main.${this.ext}`)
    );
